...
function deploy() {
   ...
   file=deployment/chaincode/connection.json.tpl
    cp deployment/chaincode/connection-chaincode.json $file

    for file in $(ls $CFG_CONFIG_PATH/chaincode/connection.json.tpl); do
    ...
    echo "> clearing chaincode templates"
    rm deployment/chaincode/connection-chaincode.json deployment/chaincode/connection.json.tpl
