...
kubectl exec fabric-ca-tools -- fabric-ca-client register --id.name user --id.secret $CFG_CA_PEERUSERPW --id.type user $CA_CLIENT_OPTS | sed 's|Password: \(.*\)|Password: *** hidden ***\r|'
echo "> enrolling peer user user CFG_PEER_USER"
kubectl exec fabric-ca-tools -- fabric-ca-client enroll -u https://user:$CFG_CA_PEERUSERPW@$CA_URL -M $CFG_USER_BASE/msp $CA_CLIENT_OPTS
...
kubectl exec fabric-ca-tools -- fabric-ca-client enroll -u https://user:$CFG_CA_PEERUSERPW@$CA_URL --enrollment.profile tls -M $CFG_USER_BASE/tls $CA_CLIENT_OPTS
