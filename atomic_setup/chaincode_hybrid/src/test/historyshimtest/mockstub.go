...
// Package historyshimtest provides a mock of the ChaincodeStubInterface for
...
func (stub *MockStub) PutState(key string, value []byte) error {
	fmt.Printf("PUT STATE LEDGER[%q] = %q\n", key, string(value))
	if stub.TxID == "" {
	...
	_, ok := stub.State[key]
	if !ok {
		// empty, thus initialise with history slice
		stub.State[key] = make([]StateEntry, 0)
	}

	// append to history
	entry := StateEntry{Value: value, TxID: stub.TxID, TxTimestamp: stub.TxTimestamp, IsDelete: false}
	stub.State[key] = append(stub.State[key], entry)
...
func (stub *MockStub) DelState(key string) error {
	...
	// check if history for given key is empty
	_, ok := stub.State[key]
	if !ok {
		// empty, throw error because we can not delete it
		return fmt.Errorf(`key [%s] not found in state`, key)
	}

	// append "deleted" to history
	entry := StateEntry{Value: nil, TxID: stub.TxID, TxTimestamp: stub.TxTimestamp, IsDelete: true}
	stub.State[key] = append(stub.State[key], entry)
...
func (stub *MockStub) GetHistoryForKey(key string) (shim.HistoryQueryIteratorInterface, error) {
	history := stub.State[key]
	if history == nil {
		return nil, fmt.Errorf(`key [%q] not found in history`, key)
	}
...
type HistoryQueryIteratorInterface interface {
	// HasNext returs true if there is more
	HasNext() bool
...
func (iter *HistoryShimIterator) Next() (*queryresult.KeyModification, error) {
	...
	res = new(queryresult.KeyModification)

	entry := iter.slice[iter.currentLoc]

	res.TxId = entry.TxID
	res.Value = entry.Value
	res.Timestamp = entry.TxTimestamp
	res.IsDelete = entry.IsDelete
